...
class AssetSortConfig extends Component {
    constructor() {
        ...
        this.state = {
            ...
            selectedNode: null,    // 被选中节点相关数据 {rank, assettypename, assettypeid, memo, rootnode}
    ...
    addAssetSort() {
        ...
        // 被选中节点的数据
        const {rank, assettypename: parenttypename, rootnode} = this.state.selectedNode
    ...
    */
    ...
    getTreeNode(treeData) {
        let nodeArr = treeData.map((v, i, arr) => {
            if (v.rank == 2) {
                return (
                    <TreeNode title={v.assettypename} key={v.rank + '|' + v.assettypename + '|' + v.assettypeid + '|' + v.memo + '|' + v.rootnode}>
    ...
    handleSelect(info) {
        ...
        info = info[0] ? info[0] : ''
        if (info.length > 0) {
            const [rank, assettypename, assettypeid, memo, rootnode] = info.split('|')
            this.state.selectedNode = { rank, assettypename, assettypeid, memo, rootnode } // 这个state不影响UI
        } else {
    ...
    /**
     * 判断被选中节点有哪些可用的按钮 this.setState.selectedNode = { rank, assettypename, assettypeid, memo, rootnode }
